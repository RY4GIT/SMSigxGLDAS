function [] = plot_seasonal2(site, plot_transday, plot_duration, savetype)
    ...
    if plot_transday
        for i = 1:size(site,1)
            ...
            g.geom_jitter(); g.set_title(sprintf('%s (piecewise est.)',site(i,:))); g.set_names('x', 'Seasonal transition dates', 'y', 'Day of the year', 'color', 'depth (cm)');
        ...
        for i = 1:size(site,1)
            ...
            g=gramm('x',cellstr(sig.type(selected_network)),'y',sig.value(selected_network));
            g.geom_jitter(); g.set_title(sprintf('%s (logistic est.)',site(i,:))); g.set_names('x', 'Seasonal transition dates', 'y', 'Day of the year', 'color', 'depth (cm)');
            g.axe_property('xdir','reverse');g.axe_property('YLim',[0 365]);
            fig2 = figure('Position',[100 100 500 300]);  g.draw(); 
    ...
    if plot_duration
        for i = 1:size(site,1)
            ...
            g=gramm('x',cellstr(sig.type(selected_network)),'y',sig.value(selected_network));
            g.geom_jitter(); g.set_title(sprintf('%s (piecewise est.)',site(i,:))); g.set_names('x', 'Season', 'y', 'Transition duration (days)', 'color', 'depth (cm)');
        ...
        for i = 1:size(site,1)
            ...
            g=gramm('x',cellstr(sig.type(selected_network)),'y',sig.value(selected_network));
            g.geom_jitter(); g.set_title(sprintf('%s (logistic est.)',site(i,:))); g.set_names('x', 'Season', 'y', 'Transition duration (days)', 'color', 'depth (cm)');
            g.axe_property('xdir','reverse');g.axe_property('YLim',[0 300]);
            fig2 = figure('Position',[100 100 350 300]);  g.draw(); 
