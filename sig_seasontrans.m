...
%             dry2wet_end2 = dry2wet_end2(1);
            ...
            if length(seasonsm.Var1) < 100
                ...
                P0 = [0.1       0.0001      0      10     wp   fc]; %[P1 P2 P3 P4 wilting_point field_capacity]
                Plb = [-1.5        0        -50       1    wp-0.5   fc-0.5];
                Pub = [0.2        0.1        180     250    wp+0.5   fc+0.5];
                ...
                if Pfit(i,1) > Pub(1) - 0.01 || (Pfit(i,3) > Pub(3)-0.01) || (Pfit(i,4) < Plb(4)+0.001) || (Pfit(i,4) > Pub(4)-0.001) || (abs(Pfit(i,6) - Pfit(i,5)) < 1.0E-04)
                    Pfit(i,:) = NaN(1,6);
                end
            ...
            %     xline(Pfit(i,3),'LineWidth',2); xline(Pfit(i,3)+Pfit(i,4),'LineWidth',2);
                ...
                Llb = [wp-0.5  fc-wp-0.5       0       -10];
                Lub = [wp+0.5  fc-wp+0.5     20       120];
                ...
                if (Lfit(i,3) == 0) ||  ( Lfit(i,4)> Lub(4)-0.01) || ( Lfit(i,4) < Llb(4)+0.01)
                    Lfit(i,:) = NaN(1,4);
                end
...
%% Wet2dry procedure
        ...
        if sum(isnan(seasonsm2.Var1))/size(seasonsm2.Var1,1) > 0.2 || isempty(seasonsm2)
            ...
            if length(seasonsm2.Var1) < 100
                ...
                P02 =   [0.1     -0.0001          20      10     wp   fc]; %[P1 P2 P3 P4 wilting_point field_capacity]
                Plb2 =  [-0.2     -0.1           -50       1    wp-0.5   fc-0.5];
                Pub2 =  [1.5    0            200      250    wp+0.5   fc+0.5];
                ...
                if Pfit2(i,1) < Plb2(1) + 0.01 || (Pfit2(i,3) > Pub2(3)-0.01) || (Pfit2(i,4) < Plb2(4)+0.001) || (Pfit2(i,4) > Pub2(4)-0.01) || (abs(Pfit2(i,6) - Pfit2(i,5)) < 1.0E-04)
                    Pfit2(i,:) = NaN(1,6);
                end
            ...
            %     xline(Pfit2(i,3),'LineWidth',2); xline(Pfit2(i,3)+Pfit2(i,4),'LineWidth',2);
                ...
                % == Logit model == %
                L02 =  [wp  fc-wp     -1E-01  50]; %[y-shift amplitude k_steepness x-shift wilting_point field_capacity]
                Llb2 = [wp-0.5  fc-wp-0.5    -10      0];
                Lub2 = [wp+0.5  fc-wp+0.5     0       200];
                ...
                if (Lfit2(i,3) > -1.0E-05) || ( Lfit2(i,4)> Lub2(4)-0.01) || ( Lfit2(i,4)< Llb2(4)+1.0E-05)
                    Lfit2(i,:) = NaN(1,4);
                end
...
%     
%     % average of all the years for the station
%     seasontrans_sdate_dry2wet_p = mean(datenum(record_seasontrans_date(:,1)),'omitnan');
%     seasontrans_edate_dry2wet_p = mean(datenum(record_seasontrans_date(:,2)),'omitnan');
%     
%     seasontrans_sdate_dry2wet_l = mean(datenum(record_seasontrans_date(:,3)),'omitnan');
%     seasontrans_edate_dry2wet_l = mean(datenum(record_seasontrans_date(:,4)),'omitnan');
%     
%     seasontrans_sdate_wet2dry_p = mean(datenum(record_seasontrans_date(:,5)),'omitnan');
%     seasontrans_edate_wet2dry_p = mean(datenum(record_seasontrans_date(:,6)),'omitnan');
%    
%     seasontrans_sdate_wet2dry_l = mean(datenum(record_seasontrans_date(:,7)),'omitnan');
%     seasontrans_edate_wet2dry_l = mean(datenum(record_seasontrans_date(:,8)),'omitnan');
%     
%     seasontrans_duration_dry2wet_p = mean(record_seasontrans_duration(:,1), 'omitnan');
%     seasontrans_duration_dry2wet_l = mean(record_seasontrans_duration(:,2), 'omitnan');
%     
%     seasontrans_duration_wet2dry_p = mean(record_seasontrans_duration(:,3), 'omitnan');
%     seasontrans_duration_wet2dry_l = mean(record_seasontrans_duration(:,4), 'omitnan');
