function [sig] = stat_align(site, depth, nstation, sig_abb, sig_abb2)
...
% This script aligns seasonal transition signatures results of twi sets of data, GLDAS and Oznet. 
  ...
  % Set path
   ...
   out_path = "G:\Shared drives\Ryoko and Hilary\SoilMoistureSignature\GLDAS\4_out\stats";
   cd("G:\Shared drives\Ryoko and Hilary\SoilMoistureSignature\GLDAS\3_codes_plotters");
    ...
    for s = 1:size(sig_abb,1)
        ...
        for i = 1:size(site,1)
            ...
            if s <= 8 
                fn = sprintf('%s2_%s.txt', sig_abb(s), site(i,:));
                fid = fopen(fullfile(in_path,fn),'r');
                disp(fid)
                sig02 = textscan(fid, '%d %d %d %d \n','HeaderLines',0, 'Delimiter',',');
                fclose(fid);
                clear fn fid
                
                sig2.network = [sig2.network; repelem(string(site(i,:)),1,length(sig02{1}))'];
                sig2.depth = [sig2.depth; sig02{1}];
                sig2.station = [sig2.station; sig02{2}];
                sig2.type = [sig2.type; repelem(sig_abb2(s),1,length(sig02{1}))'];
...
%% Align all the reuslts, seasonal transition date signatures (date)
...
%             fprintf('Currently processing the data at depth %d cm, station %d \n', depth(k), n);    
            ...
            for s = 1:8
                row3_1 = logical(sig.network == "Oznet");
                row3_2 = logical(sig.network == "GLDAS");
...
clear selected_sig_gl selected_sig_oz selected_sig_gl2 selected_sig_oz2
    ...
    %% Align all the reuslts, seasonal transition date signatures (deviation)
...
%             fprintf('Currently processing the data at depth %d cm, station %d \n', depth(k), n);    
            ...
            for s = 1:8
                row3_1 = logical(sig2.network == "Oznet");
                row3_2 = logical(sig2.network == "GLDAS");
                row4 = logical(sig2.type == sig_abb2(s));
                        ...
                        for s2 = 1:length(selected_sig_gl)
                            if char(selected_sig_gl(s2)) ~= "NaN " && char(selected_sig_gl(s2)) ~= "NaT " && char(selected_sig_oz(s2)) ~= "NaN " && char(selected_sig_oz(s2)) ~= "NaT "
                                ...
                                % if abs(datetime(selected_sig_gl(s2)) - datetime(selected_sig_oz(s2))) < month(2)
                                    ...
                                    results_timing2.type = [results_timing2.type; sig_abb2(s)];
                                    results_timing2.oz = [results_timing2.oz; selected_sig_oz(s2)];
                                    results_timing2.gl = [results_timing2.gl; selected_sig_gl(s2)]; 
                                    results_timing2.residual = [results_timing2.residual; selected_sig_gl(s2) - selected_sig_oz(s2)];
                                % end
                            end
                        end
                    end
...
results_duration.depth = []; results_duration.station = []; results_duration.type = []; results_duration.oz = []; results_duration.gl = []; results_duration.residual = [];
    ...
    % loop for the station
        for n = 1:nstation 
            fprintf('Currently processing the data at depth %d cm, station %d', depth(k), n)
...
%         "Dry to wet (p)"; "Dry to wet (l)"];
            ...
            for s4 = 9:12
                    ...
                    row3_1 = logical(sig.network == "GLDAS");
                    row4 = logical(sig.type == sig_abb2(s4));
                    row6 = row1&row2&row3_1&row4;
                    selected_sig_gl = sig.value(row6);
                    clear row3 row4 row5
